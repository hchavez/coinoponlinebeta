        ...
        <div class="panel">
            ...
            <div class="panel-body">
                ...
                <div id="exampleTableSearch_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4">
                    ...
                    <div class="row"><div class="col-sm-12">
                            <table class="table table-hover dataTable table-striped w-full dtr-inline table-responsive" id="exampleTableSearch" role="grid" aria-describedby="exampleTableSearch_info" >
                                ...
                                <tbody>
                                      ...
                                      @foreach ($machine_settings as $machine_setting)
                                        <tr role="row" class="odd">
                                          <td class="sorting_1">{{ $machine_setting->machine_id }}</td>
                                          <td>{{ $machine_setting->xTime }}</td>
                                          <td class="hidden-xs">{{ $machine_setting->yTime }}</td>
                                          <td class="hidden-xs">{{ $machine_setting->xSpeed }}</td>
                                          <td class="hidden-xs">{{ $machine_setting->zSpeed }}</td>
                                          <td>
                                            <form class="row" method="POST" action="{{ route('machine-settings.destroy', ['id' => $machine_setting->id]) }}" onsubmit = "return confirm('Are you sure?')">
                                                <input type="hidden" name="_method" value="DELETE">
                                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                                <a href="{{ route('machine-settings.edit', ['id' => $machine_setting->id]) }}" class="btn btn-warning">
                                                Update
                                                </a>
                    ...
                    <div class="row">
                        <div class="col-sm-5">
                            <div class="dataTables_info" id="example2_info" role="status" aria-live="polite">Showing 1 to {{count($machine_settings)}} of {{count($machine_settings)}} entries</div>
                        </div>
                        <div class="col-sm-7">
                            <div class="dataTables_paginate paging_simple_numbers" id="example2_paginate">
                                 {{ $machine_settings->links() }}
                            </div>
                        </div>
                    </div>
                </div>
